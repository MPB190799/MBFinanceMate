/* ===== MBFinanceMate – portfolio.js (Final Enhanced Version) =====
 * Automatische Kennzahlen:
 *  - Official Dividend Yield (TTM / Current Price)
 *  - Persönlicher YOC (TTM / AvgPrice)
 *  - Farbe: >8% Grün, 3–8% Gold, <3% Rot
 */
...
window.addPos = async function addPos(){
  try{
    ...
    if(!body.shares || !body.avgPrice || (!body.ticker && !body.isin && !body.name)){
      alert('Bitte Shares, Purchase Price und Name/ISIN/Ticker angeben.');
      return;
    }
...
function renderKPIs(){
  ...
  const avgYOCpct = (t.costBasis>0)
    ? ( (Number(t.incomeAnnual||0) / Number(t.costBasis||1)) * 100 )
    : null;
...
function renderTable(){
  ...
  state.rows.forEach(row=>{
    ...
    const officialYield = row.currentPrice > 0
      ? (row.dividendPerShareTTM / row.currentPrice) * 100
      : null;

    const offYieldClass =
      officialYield >= 6 ? "mc-up" :
      officialYield >= 3 ? "mc-flat" :
      "mc-down";

    // Persönlicher YOC
    const yocClass =
      row.yocPct >= 8 ? "mc-up" :
      row.yocPct >= 5 ? "mc-flat" :
      "mc-down";
...
async function loadAndRender(){
  ...
  state.rows   = Array.isArray(data?.portfolio) ? data.portfolio 
                : Array.isArray(data?.enriched) ? data.enriched 
                : [];
